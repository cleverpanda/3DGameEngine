...
void clip_polygon_against_plane(polygon_t* polygon, int plane) {
    ...
    tex2_t inside_texcoords[MAX_NUM_POLY_TRIANGLES];
    ...
    while (current_vertex != &polygon->vertices[polygon->num_vertices]) {
        ...
        if (current_dot * previous_dot < 0) {
            ...
            vec3_t intersection_point = {
                lerpf(previous_vertex->x,current_vertex->x,t),
                lerpf(previous_vertex->y,current_vertex->y,t),
                lerpf(previous_vertex->z,current_vertex->z,t)
            };

            tex2_t interpolated_texcoord = {
                lerpf(previous_texcoord->u,current_texcoord->u,t),
                lerpf(previous_texcoord->u,current_texcoord->u,t)
            };
